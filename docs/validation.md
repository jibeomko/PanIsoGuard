...
| [multicaller](../benchmark/multicaller) | **three** callers (FLAIR + IsoQuant + Bambu) on one shared alignment, integrated by `combine`, scored vs SQANTI-SIM truth | single-caller novel precision **0.349**, ≥2-caller consensus **0.979** (recall 0.788); per-caller precision varies 0.74–0.98 — agreement is the caller-agnostic signal. Engine reproduces it: `adjudicate --caller-support` (long-read only) takes FLAIR from **0 confident novels (all AMBIGUOUS) → 0.978 precision** by promoting cross-caller-agreed novels |
...
| **LRGASP real data** ([lrgasp/](../benchmark/lrgasp)) | blocked | The LRGASP pre-run caller GTFs are **Synapse-gated**. The multi-caller capability is already validated on genuine FLAIR + IsoQuant + Bambu output, truth-scored (see `multicaller`). |
