...
| [hg03516_refbias](../benchmark/hg03516_refbias) | the rescue on **real PacBio Iso-Seq from a divergent African individual** (HG03516, ESN/Nigeria; HPRC R2 — same-individual RNA + HiFi assembly) | **POSITIVE real-data application** (the differentiator firing on real RNA calls). Of 98,672 IsoQuant novel junctions, **40 are reference-bias** (non-canonical on GRCh38, canonical on HG03516's own haplotype via a real assembly SNV), **35 novel vs GENCODE**; 40/40 trace to a real SNV + read-supported, **40/40 rescued (0 false)**, **40/40 firewall-held**. vs reference-grade GM12878 **0** — *directional, not a controlled rate* (denominator/chemistry/variant-method differ; the apples-to-apples scaling evidence is the cohort above). Independently adversarially audited (dropped 1 displaced-coordinate hit; assembly-SNV / no-indel / placeholder-QC caveats stated) |
